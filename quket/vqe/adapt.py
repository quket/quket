...
from quket.utils import Gdoubles_list, cost_mpi, jac_mpi_num, jac_mpi_ana
...
from quket.opelib import Gdouble_ope, Gdouble_fswap
from quket.ansatze import create_exp_state, create_exp_intermediate_states
...
def adapt_vqe_driver(Quket, opt_method, opt_options, print_level, maxiter):
    ...
    while True:  ##for ADAPT-VQE
        ...
        if (maxiter > 0 and not Quket.adapt.lucc_nvqe) or (vqe_cyc < Quket.adapt.svqe and Quket.adapt.lucc_nvqe):
            ...
            create_state = lambda theta_list, init_state: create_adapt_state_pauli(Quket,theta_list, init_state=Quket.init_state, intermediate_states=False)[0]
            if Quket.constraint_lambda_Sz > 0:
                jac_wrap_mpi = lambda theta_list: jac_mpi_num(cost_wrap_serial, theta_list)
                #jac_wrap_mpi = None
                #jac_wrap_mpi = lambda theta_list: jac_mpi_ana(create_state, Quket, theta_list)
            else:
        ...
        cput = t2 - t1
        state, circuit_depth = create_adapt_state_pauli(Quket,theta_list, intermediate_states=False)
        prints(
...
def create_adapt_state_pauli(Quket, theta_list, depth=None, init_state=None, intermediate_states=False):
    """Function
    ...
    if theta_list is not None:
        ...
        if intermediate_states:
            ...
            #state = create_exp_state(Quket, init_state=init_state, pauli_list=Quket.pauli_list[:depth], theta_list=theta_list[:depth], rho=1)
            state, _intermediate_state =  create_exp_intermediate_states(Quket, theta_list=theta_list[:depth], pauli_list=Quket.pauli_list[:depth], init_state=init_state)
        else:
            #Quket.cf.create_w_1process = False
            state = create_exp_state(Quket, init_state =init_state, theta_list=theta_list[:depth], pauli_list=Quket.pauli_list[:depth], rho=1)
...
def cost_adapt_pauli(theta_list, Quket, parallel=True):
    ...
    """
    state, dummy = create_adapt_state_pauli(Quket, theta_list, intermediate_states=False)
    Quket.state = state
...
def calculate_spin_adapt_gradient_pauli(Quket, theta_list, pauli_list, fullspin=False):
    ...
    state, dummy = create_adapt_state_pauli(Quket, theta_list, depth=ndim-1, intermediate_states=False)
